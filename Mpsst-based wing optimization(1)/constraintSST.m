function const = constraintSST(individual,minlayer,difthick)
...
try
    ...
    SST = sequence(individual, minlayer, difthick);
     ...
     % 如果发生任何错误，捕获并输出 const = 0
     %warning('An error occurred while calling sequence function. Returning const = 0.');
     const = 0;
     return; % 中止函数执行
 end
const = 1;
